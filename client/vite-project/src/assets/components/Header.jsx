...
const Header = () => {
  return (
    <div className='flex flex-col justify-center items-center text-center my-20'>
      ...
      <h1 className='text-4xl max-w-[300px] sm:text-7xl sm:max-w-[590px] max-auto mt-10 text-center'>Turn text to <span className='text-blue-600'>image</span>, in seconds. </h1>
      ...
      <div>
        {Array(6).fill('').map((item , index)=>(
          <img className='rounded hover:scale-105 transition-all duration-300 cursor-pointer max-sm:w-10' src={assets.sample_img_1} key={index} width={70}/>
        ))}
      </div>
    </div>
  )
